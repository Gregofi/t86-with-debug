...
4 HALT
    ...
    ASSERT_THROW({ native.ReadText(0, 6); }, DebuggerError);
    ASSERT_THROW({ native.ReadText(5, 1); }, DebuggerError);
    ASSERT_NO_THROW({ native.ReadText(4, 1); });
    ...
    ASSERT_THROW({ native.GetRegister("R3"); }, DebuggerError);
...
4 HALT
    ...
    ASSERT_THROW({ native.WriteText(4, { "HALT", "HALT" }); }, DebuggerError);
    ASSERT_NO_THROW({ native.WriteText(4, { "HALT" }); });

    native.SetRegister("R0", 1);
    ASSERT_THROW({ native.SetRegister("R3", 2); }, DebuggerError);
