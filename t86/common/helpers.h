...
#define UNREACHABLE                                                           \
    throw std::runtime_error(                                                 \
        STR("Unreachable code at " << __FILE__ << ": " << __LINE__))
#define NOT_IMPLEMENTED                                                       \
    throw std::runtime_error(                                                 \
        STR("Not implemented code at " << __FILE__ << ":" << __LINE__))

namespace tiny {
template <typename T> std::unique_ptr<T> to_unique(T* ptr) {
...
inline std::vector<std::string_view> split_v(
...
inline std::string join(
    InputIt begin, InputIt end, const std::string& j = " ") {
    std::string res;
    for (; begin < end; ++begin) {
        res += *begin;
