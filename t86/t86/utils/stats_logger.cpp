...
void StatsLogger::newTick() { ticks_.emplace_back(); }

std::size_t StatsLogger::tickCount() const { return ticks_.size(); }
...
void StatsLogger::processBasicStats(std::ostream& os) {
    ...
    std::map<Instruction::Signature,
        std::pair<InstructionLifeTime, std::size_t>>
        lifetimesBySignature;
    ...
    for (const auto& [id, ins] : instructions_) {
        ...
        auto& signatureEntry
            = lifetimesBySignature[instructions_.at(id)
                                       .second->getSignature()];
    ...
    os << "Global averages:\n";
    processAverageLifetime(
        os, accumulativeInstructionLifeTime, totalInstructions);
    std::cerr << std::flush;
}
...
StatsLogger::TickStats& StatsLogger::currentTick() { return ticks_.back(); }
...
void StatsLogger::logStallFloatRegisterFetch(
    std::size_t id, FloatRegister fReg) {
...
void StatsLogger::processAverageLifetime(std::ostream& os,
    const StatsLogger::InstructionLifeTime& lt, std::size_t totalCount) {
    os << "  Times executed: " << totalCount << '\n'
       << "  Average instruction lifetime: "
       << static_cast<double>(lt.totalTime()) / totalCount << " ticks\n"
       << "    Average fetch stalls: "
       << static_cast<double>(lt.fetch - 1) / totalCount << " ticks\n"
       << "    Average decode stalls: "
       << static_cast<double>(lt.decode - 1) / totalCount << " ticks\n"
       << "    Average operand fetching stalls: "
       << static_cast<double>(lt.fetchingStalls) / totalCount << " ticks\n";
    ...
    if (totalWaitingForRegisters != 0) {
        os << "      Average register fetch stalls: "
           << static_cast<double>(totalWaitingForRegisters) / totalCount
           << " ticks\n";
    }
    ...
    if (totalWaitingForFloatRegisters != 0) {
        os << "      Average float register fetch stalls: "
           << static_cast<double>(totalWaitingForFloatRegisters) / totalCount
           << " ticks\n";
    }
    ...
    if (totalWaitingForMemory != 0) {
        os << "      Average memory read stalls: "
           << static_cast<double>(totalWaitingForMemory) / totalCount
           << " ticks\n";
    }

    os << "    Average waiting for ALU: "
       << static_cast<double>(lt.waitingForAlu) / totalCount << " ticks\n"
       << "    Average executing: "
       << static_cast<double>(lt.executing) / totalCount << " ticks\n"
       << "    Average waiting for retirement: "
       << static_cast<double>(lt.waitingForRetirement) / totalCount
       << " ticks\n"
       << "    Average retirement: "
       << static_cast<double>(lt.retirement) / totalCount << " ticks\n";
}

std::size_t StatsLogger::registerNewInstruction(
...
StatsLogger::InstructionLifeTime StatsLogger::getInstructionLifeTime(
    std::size_t id) {
    ...
    while (it != ticks_.end() &&
        ...
        std::find(it->operandFetchingRSEntries.begin(),
            it->operandFetchingRSEntries.end(), id)
            != it->operandFetchingRSEntries.end()) {
        ++lifeTime.preparing;
        if (std::find(it->operandFetchingStallRSEntries.begin(),
                it->operandFetchingStallRSEntries.end(), id)
            != it->operandFetchingStallRSEntries.end()) {
            ++lifeTime.fetchingStalls;
        }
        if (auto rsIt = it->stallRegisterFetchRSEntries.find(id);
            rsIt != it->stallRegisterFetchRSEntries.end()) {
            for (Register reg : rsIt->second) {
                ++lifeTime.waitingForRegisterFetch[reg];
            }
        }
        if (auto frsIt = it->stallFloatRegisterFetchRSEntries.find(id);
            frsIt != it->stallFloatRegisterFetchRSEntries.end()) {
            for (FloatRegister fReg : frsIt->second) {
                ++lifeTime.waitingForFloatRegisterFetch[fReg];
            }
        }
        if (auto memIt = it->stallRAMReadRSEntries.find(id);
            memIt != it->stallRAMReadRSEntries.end()) {
    ...
    while (it != ticks_.end()
        && std::find(it->stallNoAluRSEntries.begin(),
               it->stallNoAluRSEntries.end(), id)
            != it->stallNoAluRSEntries.end()) {
        ++lifeTime.waitingForAlu;
        ++it;
    }
    while (it != ticks_.end()
        && std::find(it->executingRSEntries.begin(),
               it->executingRSEntries.end(), id)
            != it->executingRSEntries.end()) {
        ++lifeTime.executing;
        ++it;
    }
    while (it != ticks_.end()
        && std::find(it->stallRetirementRSEntries.begin(),
               it->stallRetirementRSEntries.end(), id)
            != it->stallRetirementRSEntries.end()) {
        ++lifeTime.waitingForRetirement;
        ++it;
    }
    assert(it != ticks_.end());
    assert(
        std::find(it->retiredRSEntries.begin(), it->retiredRSEntries.end(), id)
        != it->retiredRSEntries.end());
    ++lifeTime.retirement;
    return lifeTime;
}
} // namespace tiny::t86
