...
public:
    ...
    std::size_t registerNewInstruction(
    ...
    struct TickStats {
        ...
        std::map<std::size_t, std::set<FloatRegister>>
            stallFloatRegisterFetchRSEntries;
...
protected:
    ...
    struct InstructionLifeTime {
        ...
        std::size_t retirement {
            0
        }; // Every retirement will take only one cycle, so
           // this is excess for now, but maybe in future?

        std::size_t totalTime() const {
            return fetch + decode + preparing + waitingForAlu + executing
                + waitingForRetirement + retirement;
        }
        ...
        InstructionLifeTime& operator+=(const InstructionLifeTime& other) {
            ...
            for (const auto& [reg, count] :
                other.waitingForFloatRegisterFetch) {
    ...
    static void processAverageLifetime(std::ostream& os,
        const InstructionLifeTime& lt, std::size_t totalCount);
    ...
    std::unordered_map<std::size_t, std::pair<std::size_t, const Instruction*>>
        instructions_;
};
} // namespace tiny::t86
