...
Register Pc() { return Register::ProgramCounter(); }

Register Flags() { return Register::Flags(); }

Register Sp() { return Register::StackPointer(); }

Register Bp() { return Register::StackBasePointer(); }

FloatRegister FReg(size_t index) { return FloatRegister(index); }

Memory::Register Mem(Register reg) { return Memory::Register(reg); }

Memory::Immediate Mem(uint64_t index) { return Memory::Immediate(index); }
...
Memory::RegisterOffsetRegister Mem(
    const RegisterOffsetRegister& regOffsetReg) {
    return Memory::RegisterOffsetRegister(regOffsetReg);
}

Memory::RegisterRegisterScaled Mem(
    const RegisterRegisterScaled& regRegScaled) {
