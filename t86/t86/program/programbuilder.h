...
public:
    ProgramBuilder(bool release = false)
        : release_(release) { }
    ...
    template <typename T> Label add(const T& instruction) {
    ...
    // This returns current Label, this label will point to next added
    // instruction
    Label currentLabel() const { return instructions_.size(); }
    ...
    void patch(Label instruction, Label destination) {
        ...
        auto* jmpInstruction
            = dynamic_cast<PatchableJumpInstruction*>(ins.get());
