...
public:
    ...
    public:
        ...
        constexpr static const char* registerCountConfigString
            = "-registerCnt";

        constexpr static std::size_t defaultRegisterCount = 10;

        constexpr static const char* floatRegisterCountConfigString
            = "-floatRegisterCnt";
        ...
        constexpr static const char* reservationStationEntriesCountConfigString
            = "-reservationStationEntriesCnt";
    ...
    // Max instruction operands - for example ADD R1 R2 has 3 (destination and
    // 2 source)
    ...
    // This is how many renamed registers can be in use translate once for once
    // instruction
    //  (it is quite generous, usually around 3 will be used translate once)
    static constexpr std::size_t possibleRenamedRegisterCnt
        = maxInstructionOperands + specialRegistersCnt;
    ...
    Cpu(std::size_t registerCount, std::size_t floatRegisterCount,
        std::size_t aluCnt);
    ...
    std::size_t registersCount() const { return registerCnt_; }

    std::size_t physicalRegistersCount() const { return physicalRegisterCnt_; }
    ...
    std::optional<uint64_t> readMemory(
...
private:
    ...
    /// resets the value of trap flag. They are also set as part of entity,
    /// which makes it harder because it is done somewhere in the execution
    /// pipeline and is non-transparent.
    bool trapFlag_ { false };

    // Branch processing
    void checkBranchPrediction(
