...
RAM::RAM(std::size_t memSize, std::size_t gatesCnt)
    : mem_(memSize, 0)
    , gatesCnt_(gatesCnt) { }
...
std::optional<int64_t> RAM::read(std::size_t address) {
    ...
    if (!isBusy()) {
        ...
        assert(writes_.find(address) == writes_.end()
            && "You should not read from address that is being written to");
...
bool RAM::isBusy() const { return reads_.size() == gatesCnt_; }
...
RAM::WriteId RAM::write(std::size_t address, int64_t value) {
    ...
    writesById_.insert(std::make_pair(id,
        std::cref(writes_[address]
            = WriteEntry { id, writeLatency(address), value })));
    return id;
}

int64_t RAM::get(std::size_t address) const { return mem_.at(address); }

void RAM::set(std::size_t address, int64_t value) { mem_.at(address) = value; }

std::size_t RAM::size() const { return mem_.size(); }
