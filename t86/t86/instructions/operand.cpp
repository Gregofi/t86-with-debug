...
Operand::Operand(const RegisterOffset& regOff)
    : value_(regOff) { }

Operand::Operand(const RegisterRegister& regReg)
    : value_(regReg) { }

Operand::Operand(const RegisterScaled& regScaled)
    : value_(regScaled) { }

Operand::Operand(const RegisterOffsetRegister& regOffReg)
    : value_(regOffReg) { }

Operand::Operand(const RegisterRegisterScaled& regRegScaled)
    : value_(regRegScaled) { }

Operand::Operand(const RegisterOffsetRegisterScaled& regOffRegScaled)
    : value_(regOffRegScaled) {};

Operand::Operand(const Memory::Immediate& address)
    : value_(address) { }

Operand::Operand(const Memory::Register& address)
    : value_(address) { }

Operand::Operand(const Memory::RegisterOffset& address)
    : value_(address) { }

Operand::Operand(const Memory::RegisterRegister& address)
    : value_(address) { }

Operand::Operand(const Memory::RegisterScaled& address)
    : value_(address) { }

Operand::Operand(const Memory::RegisterOffsetRegister& address)
    : value_(address) { }

Operand::Operand(const Memory::RegisterRegisterScaled& address)
    : value_(address) { }

Operand::Operand(const Memory::RegisterOffsetRegisterScaled& address)
    : value_(address) { }

Operand::Operand(const FloatRegister& fReg)
    : value_(fReg) { }
...
bool Operand::isMemoryRegisterOffsetRegisterScaled() const {
    return std::holds_alternative<Memory::RegisterOffsetRegisterScaled>(
        value_);
}
...
const RegisterOffsetRegisterScaled&
Operand::getRegisterOffsetRegisterScaled() const {
...
const Memory::RegisterOffsetRegister&
Operand::getMemoryRegisterOffsetRegister() const {
    assert(isMemoryRegisterOffsetRegister());
    return std::get<Memory::RegisterOffsetRegister>(value_);
}

const Memory::RegisterRegisterScaled&
Operand::getMemoryRegisterRegisterScaled() const {
    assert(isMemoryRegisterRegisterScaled());
    return std::get<Memory::RegisterRegisterScaled>(value_);
}

const Memory::RegisterOffsetRegisterScaled&
Operand::getMemoryRegisterOffsetRegisterScaled() const {
...
bool Operand::isFetched() const { return isValue() || isFloatValue(); }
...
Requirement Operand::requirement() const {
    ...
    if (isRegister()) {
        ...
        return RegisterRead(
            getRegisterOffsetRegisterScaled().regScaled().reg());
    } else if (isMemoryImmediate()) {
        ...
        return RegisterRead(getMemoryRegisterOffsetRegister()
                                .regOffsetReg()
                                .regOffset()
                                .reg());
    } else if (isMemoryRegisterRegisterScaled()) {
        return RegisterRead(getMemoryRegisterRegisterScaled()
                                .regRegScaled()
                                .regScaled()
                                .reg());
    } else if (isMemoryRegisterOffsetRegisterScaled()) {
        return RegisterRead(getMemoryRegisterOffsetRegisterScaled()
                                .regOffsetRegScaled()
                                .regScaled()
                                .reg());
...
int64_t Operand::supply(const Register&, int64_t val) { return val; }
...
RegisterOffset Operand::supply(
    const RegisterOffsetRegister& reg, int64_t val) {
    return RegisterOffset(reg.reg(), reg.regOffset().offset() + val);
}

RegisterOffset Operand::supply(
    const RegisterRegisterScaled& reg, int64_t val) {
    return RegisterOffset(reg.reg(), reg.regScaled().scale() * val);
}

RegisterOffset Operand::supply(
    const RegisterOffsetRegisterScaled& reg, int64_t val) {
    return RegisterOffset(reg.regOffset().reg(),
        reg.regOffset().offset() + reg.regScaled().scale() * val);
}

int64_t Operand::supply(const Memory::Immediate&, int64_t val) { return val; }

Memory::Immediate Operand::supply(const Memory::Register& mem, int64_t val) {
    return Memory::Immediate(val);
}

Memory::Immediate Operand::supply(
    const Memory::RegisterOffset& mem, int64_t val) {
    return Memory::Immediate(supply(mem.regOffset(), val));
}

Memory::RegisterOffset Operand::supply(
    const Memory::RegisterRegister& mem, int64_t val) {
    return Memory::RegisterOffset(supply(mem.regReg(), val));
}

Memory::Immediate Operand::supply(
    const Memory::RegisterScaled& mem, int64_t val) {
    return Memory::Immediate(supply(mem.regScaled(), val));
}

Memory::RegisterOffset Operand::supply(
    const Memory::RegisterOffsetRegister& mem, int64_t val) {
    return Memory::RegisterOffset(supply(mem.regOffsetReg(), val));
}

Memory::RegisterOffset Operand::supply(
    const Memory::RegisterRegisterScaled& mem, int64_t val) {
    return Memory::RegisterOffset(supply(mem.regRegScaled(), val));
}

Memory::RegisterOffset Operand::supply(
    const Memory::RegisterOffsetRegisterScaled& mem, int64_t val) {
    return Memory::RegisterOffset(supply(mem.regOffsetRegScaled(), val));
}
double Operand::supply(const FloatRegister&, double val) { return val; }
} // namespace tiny::t86
