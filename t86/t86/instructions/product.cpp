...
Product Product::fromOperand(Operand op) {
    ...
    } else if (op.isMemoryRegister() || op.isMemoryRegisterOffset()
        || op.isMemoryRegisterScaled() || op.isMemoryRegisterRegister()
    ...
    } else if (op.isRegisterOffset() || op.isRegisterScaled()
        || op.isRegisterRegister() || op.isRegisterOffsetRegister()
        || op.isRegisterRegisterScaled()
        || op.isRegisterOffsetRegisterScaled()) {
        throw std::runtime_error("Trying to convert operand to product, that "
                                 "can be only used as value");
    }
