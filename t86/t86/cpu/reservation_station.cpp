...
void ReservationStation::executeAndRetire() {
    ...
    // We start from the start and retire until first unfinished instruction is
    // met It is better to do this in this while loop, because retiring
    // conditional jump instruction might lead to erasure of all other
    // instructions in reservation station (invalidating all iterators)
    ...
    for (auto& entry : entries_) {
        ...
        case Entry::State::preparing: {
            ...
            for (Operand& operand : entry.operands()) {
                // Check if we need to fetch and fetch as much as we can right
                // now
                while (!operand.isFetched()) {
                    ...
                    if (requirement.isRegisterRead()) {
                        ...
                        if (entry.registerAvailable(reg)) {
                            ...
                            // This following is very hacky, it's here only for
                            // better logging This assumes that there are max 2
                            // register in operands We don't really care for
                            // memory, the mem will not start fetching until we
                            // know the address, that can be made of 2
                            // registers COPY the operand, not to mess up the
                            // real operand
                            Operand op = operand;
                            // supply dummy value
                            op.supply((int64_t)0);
                        ...
                        FloatRegister fReg
                            = requirement.getFloatRegisterRead();
        ...
        case Entry::State::retiring:
            // If there are some entries at this point, it means that there are
            // some instructions before, that "block" this instruction from
            // retirement
...
ReservationStation::ReservationStation(
    Cpu& cpu, std::size_t aluCnt, std::size_t maxEntriesCnt)
    : maxEntries_(maxEntriesCnt)
    , cpu_(cpu)
    , freeAlus_(aluCnt) { }

void ReservationStation::add(const Instruction* instruction,
    std::size_t nextPc, std::size_t loggingId) {
    assert(entries_.size() < maxEntries_
        && "Can't add another entry, max capacity was reached");
    ...
    for (const auto& product : instruction->produces()) {
        if (product.isRegister()) {
            ...
            memWriteIds.push_back(
                cpu_.registerPendingWrite(product.getMemoryImmediate()));
    ...
    auto& entry = entries_.emplace_back(instruction, cpu_, std::move(readRat),
        std::move(writeRat), std::move(memWriteIds), cpu_.currentMaxWriteId(),
        loggingId);
...
bool ReservationStation::Entry::floatRegisterAvailable(
...
void ReservationStation::Entry::setRegister(Register reg, int64_t val) {
    assert(reg == Register::ProgramCounter()
        || !cpu_.registerReady(writeRat_.translate(reg)));
    cpu_.setRegister(writeRat_.translate(reg), val);
}

void ReservationStation::Entry::setFloatRegister(
    FloatRegister fReg, double val) {
...
ReservationStation::Entry::Entry(const Instruction* instruction, Cpu& cpu,
    ...
    std::vector<MemoryWrite::Id> memWriteIds, MemoryWrite::Id maxWriteId,
    ...
    remainingExecutionTime_
        = Cpu::Config::instance().getExecutionLength(instruction);
}
...
bool ReservationStation::Entry::executionTick() {
    ...
    // This is done "two-steps" because some instructions might have zero
    // execution tickCount required
    if (remainingExecutionTime_ == 0) {
...
void ReservationStation::Entry::startExecution() {
    assert(state_ == State::ready
        && "Starting execution on instruction that is not in ready state");
    state_ = State::executing;
}

void ReservationStation::Entry::checkReady() {
    assert(state_ == State::preparing
        && "Instruction should be in preparing state while checking if ready");
...
Cpu& ReservationStation::Entry::cpu() const { return cpu_; }
...
const RegisterAllocationTable& ReservationStation::Entry::rat() const {
    ...
    // Used mostly when fixing branch prediction miss, so we care mainly for
    // the PC
    return writeRat_;
}
...
std::optional<int64_t> ReservationStation::Entry::readMemory(
    uint64_t address) {
    return cpu_.readMemory(address, maxWriteId_);
}

void ReservationStation::Entry::specifyWriteAddress(
    MemoryWrite::Id id, std::size_t address) {
    cpu_.specifyWriteAddress(id, address);
}

void ReservationStation::Entry::setWriteValue(
    MemoryWrite::Id id, uint64_t value) {
...
void ReservationStation::Entry::logStallFloatRegisterFetch(
    FloatRegister fReg) const {
