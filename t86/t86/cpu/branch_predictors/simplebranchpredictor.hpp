...
{ auto it = records_.find(pc); if (it == records_.end()) { return false;
        }
        const branches& branches_ = it->second;
        size_t falseCnt = std::count_if(branches_.begin(), branches_.end(),
[](const std::optional<size_t>& branch) { return !branch.has_value();
        });

        size_t trueCnt = std::count_if(branches_.begin(), branches_.end(),
[](const std::optional<size_t>& branch) { return branch.has_value();
        });
    ...
    void registerBranchTaken(size_t pc, const Instruction&, size_t destination)
override { auto& branches_ = records_[pc]; while(branches_.size() >= N) {
            branches_.pop_front();
        }
