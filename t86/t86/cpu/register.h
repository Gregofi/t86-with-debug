...
// This one will not be a "full" operand, only for ease of writing
// RegisterOffsetRegisterScaled
class RegisterScaledOffset;

/** The register does not store its value, but it is a register descriptor that
 * tells me the index of the register only. This represents the logical
 * register (those, that is used in ISA)
 */
class Register {
public:
    ...
    constexpr Register(size_t index)
        : index_ { index } { }
    ...
    bool operator<(Register other) const { return index_ < other.index_; }
...
class RegisterOffset {
public:
    RegisterOffset(Register reg, int64_t offset)
        : reg_(reg)
        , offset_(offset) { }
...
class RegisterScaled {
public:
    RegisterScaled(const Register& reg, int64_t scale)
        : reg_(reg)
        , scale_(scale) {};
...
class RegisterRegister {
public:
    RegisterRegister(Register reg1, Register reg2)
        : reg1_(reg1)
        , reg2_(reg2) {};

    Register reg1() const { return reg1_; }

    Register reg2() const { return reg2_; }
...
class RegisterOffsetRegister {
public:
    RegisterOffsetRegister(RegisterOffset regOffset, Register reg)
        : regOffset_(regOffset)
        , reg_(reg) { }

    Register reg() const { return reg_; }

    RegisterOffset regOffset() const { return regOffset_; }
...
class RegisterRegisterScaled {
public:
    RegisterRegisterScaled(Register reg, RegisterScaled regScaled)
        : reg_(reg)
        , regScaled_(regScaled) { }

    Register reg() const { return reg_; }

    RegisterScaled regScaled() const { return regScaled_; }
...
public:
    RegisterOffsetRegisterScaled(
        RegisterOffset regOffset, RegisterScaled regScaled)
        : regOffset_(regOffset)
        , regScaled_(regScaled) { }
...
// This one is temporary class, if you try to use this as instruction operand,
// this will fail
class RegisterScaledOffset {
public:
    RegisterScaledOffset(RegisterScaled regScaled, int64_t offset)
        : regScaled_(regScaled)
        , offset_(offset) { }

    // Because this is temporary, no getters will be exposed, no one should be
    // accessing any of them anyway
 ...
 * For example this would arise if CPU has only 5 logical registers and
 * instructions uses register 8
 */
class InvalidRegister : public std::runtime_error {
public:
    InvalidRegister(Register reg)
        : std::runtime_error("Invalid register " + reg.toString()) { }
};

class FloatRegister {
public:
    FloatRegister(size_t index)
        : index_(index) { }
    ...
    std::string toString() const { return "FReg" + std::to_string(index_); }
