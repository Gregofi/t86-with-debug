...
RegisterAllocationTable::RegisterAllocationTable(
    ...
    table_.insert_or_assign(
        Register::ProgramCounter(), PhysicalRegister { ++i });
    table_.insert_or_assign(
        Register::StackPointer(), PhysicalRegister { ++i });
    table_.insert_or_assign(
        Register::StackBasePointer(), PhysicalRegister { ++i });
    table_.insert_or_assign(Register::Flags(), PhysicalRegister { ++i });
    subscribeToReads();
}

RegisterAllocationTable::RegisterAllocationTable(
    const RegisterAllocationTable& other)
    : table_ { other.table_ }
    , cpu_ { other.cpu_ } {
    subscribeToReads();
}
...
RegisterAllocationTable::RegisterAllocationTable(
    RegisterAllocationTable&& other)
    : table_ { std::move(other.table_) }
    , cpu_ { other.cpu_ } { }

RegisterAllocationTable::~RegisterAllocationTable() { unsubscribeFromReads(); }
...
RegisterAllocationTable& RegisterAllocationTable::operator=(
    const RegisterAllocationTable& other) {
...
bool RegisterAllocationTable::isUnmapped(PhysicalRegister reg) const {
    return std::all_of(
        table_.begin(), table_.end(), [reg](const auto& mapping) {
