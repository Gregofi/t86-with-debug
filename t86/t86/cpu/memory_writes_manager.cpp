...
void MemoryWritesManager::specifyAddress(
    MemoryWrite::Id id, std::size_t address) {
    std::size_t erased = unspecifiedWrites_.erase(id);
    assert(erased == 1
        && "Trying to specify address for invalid, unknown or "
           "already specified write id");
    writesById.emplace(id, writesMap_[address].add(id, address));
}

void MemoryWritesManager::specifyValue(
    MemoryWrite::Id id, uint64_t value) const {
    auto& write = getWrite(id);
    write.setValue(value);
}

std::optional<MemoryWrite> MemoryWritesManager::previousWrite(
    std::size_t address, MemoryWrite::Id maxId) const {
