...
void Cpu::tick() {
    log_debug(
        "main: flag register value: {:x}", getRegister(Register::Flags()));
    ...
    if (instructionDecode_) {
        if (reservationStation_.hasFreeEntry()) {
            reservationStation_.add(instructionDecode_->instruction,
                instructionDecode_->pc, instructionDecode_->loggingId);
    ...
    if (instructionFetch_) {
        StatsLogger::instance().logInstructionFetch(
            instructionFetch_->loggingId);
    }
    if (instructionDecode_) {
        StatsLogger::instance().logInstructionDecode(
            instructionDecode_->loggingId);
    }
...
Cpu::InstructionEntry Cpu::fetchInstruction() {
    ...
    if (auto jumpInstruction
        = dynamic_cast<const JumpInstruction*>(instruction);
        jumpInstruction) {
        speculativeProgramCounter_ = branchPredictor_->nextGuess(
            speculativeProgramCounter_, *jumpInstruction);
        predictions_.push_back(speculativeProgramCounter_);
    } else {
        ++speculativeProgramCounter_;
    }
    return { instruction, oldPc + 1,
        StatsLogger::instance().registerNewInstruction(oldPc, instruction) };
}
...
Cpu::Cpu(std::size_t registerCount, std::size_t floatRegisterCount,
    std::size_t aluCnt)
    : Cpu(registerCount, floatRegisterCount, aluCnt, aluCnt * 2,
        Config::instance().ramSize(), Config::instance().ramGatesCount()) { }

Cpu::Cpu(std::size_t registerCount, std::size_t floatRegisterCount,
    std::size_t aluCnt, std::size_t reservationStationEntriesCount,
    ...
    , physicalRegisterCnt_(specialRegistersCnt + registerCount
          + floatRegisterCount
          + reservationStationEntriesCount * possibleRenamedRegisterCnt)
...
void Cpu::setRegister(PhysicalRegister reg, double value) {
    registers_.at(reg.index()).value
        = *reinterpret_cast<int64_t*>(&value); // Store the double as int64_t
    registers_.at(reg.index()).ready = true;
}
...
int Cpu::interrupted() const { return interrupted_; }

void Cpu::interrupt(int code) { interrupted_ = code; }

std::optional<uint64_t> Cpu::readMemory(
    uint64_t address, MemoryWrite::Id maxId) {
...
int64_t Cpu::getMemory(uint64_t address) const { return ram_.get(address); }
...
void Cpu::jump(const ReservationStation::Entry& entry, bool taken) {
    ...
    if (taken) {
        registerBranchTaken(
            entry.getRegister(Register::ProgramCounter()), destination);
    } else {
...
void Cpu::checkBranchPrediction(
    const ReservationStation::Entry& entry, uint64_t destination) {
...
const RegisterAllocationTable& Cpu::getRat() const { return rat_; }
...
PhysicalRegister Cpu::nextFreeRegister() const {
    for (std::size_t i = 0; i < physicalRegisterCnt_; ++i) {
        if (rat_.isUnmapped(PhysicalRegister { i })
            && registers_.at(i).subscribedReads == 0) {
            return i;
        }
    }
    throw std::runtime_error(
        "No free register was found, either bug in RAT or "
        "small scale for physical registers");
}
...
void Cpu::flushPipeline() {
    ...
    predictions_.clear();
    if (instructionFetch_) {
        StatsLogger::instance().logClearSpeculation(
            instructionFetch_->loggingId);
...
Cpu::Cpu()
    : Cpu(Cpu::Config::instance().registerCnt(),
        Cpu::Config::instance().aluCnt(),
...
std::size_t Cpu::Config::getExecutionLength(const Instruction* ins) const {
    static std::map<Instruction::Signature, std::size_t> lengths = {
        { { Instruction::Type::MOV,
              { Operand::Type::Reg, Operand::Type::Imm } },
            2 },
    };
...
Cpu::Config::Config() {
    ...
    config.setDefaultIfMissing(
        Config::aluCountConfigString, std::to_string(Config::defaultAluCount));
    config.setDefaultIfMissing(
        Config::reservationStationEntriesCountConfigString,
        std::to_string(Config::defaultReservationStationEntriesCount));
    config.setDefaultIfMissing(
        Config::ramSizeConfigString, std::to_string(Config::defaultRamSize));
...
bool Cpu::singleStepDone() const { return single_stepped_; }

void Cpu::singleStepped() { single_stepped_ = true; }
} // namespace tiny::t86
