...
bool Instruction::Signature::operator<(
    const Instruction::Signature& other) const {
    return std::tie(type, operandTypes)
        < std::tie(other.type, other.operandTypes);
}
...
void BinaryArithmeticInstruction::execute(
    ReservationStation::Entry& entry) const {
...
#define BINARY_ARITH_INS_IMPL(INS_NAME, OP)                                   \
    INS_NAME::INS_NAME(Register reg, Register val)                            \
        : BinaryArithmeticInstruction(OP, reg, val) { }                       \
    INS_NAME::INS_NAME(Register reg, RegisterOffset regDisp)                  \
        : BinaryArithmeticInstruction(OP, reg, regDisp) { }                   \
    INS_NAME::INS_NAME(Register reg, int64_t val)                             \
        : BinaryArithmeticInstruction(OP, reg, val) { }                       \
    INS_NAME::INS_NAME(Register reg, Memory::Immediate val)                   \
        : BinaryArithmeticInstruction(OP, reg, val) { }                       \
    INS_NAME::INS_NAME(Register reg, Memory::Register val)                    \
        : BinaryArithmeticInstruction(OP, reg, val) { }                       \
    INS_NAME::INS_NAME(Register reg, Memory::RegisterOffset val)              \
        : BinaryArithmeticInstruction(OP, reg, val) { }                       \
    INS_NAME::INS_NAME(Register reg, Operand val)                             \
        : BinaryArithmeticInstruction(OP, reg, val) { }                       \
    INS_NAME::INS_NAME(Register dest, Register reg, int64_t val)              \
        : BinaryArithmeticInstruction(OP, dest, reg, val) { }                 \
    INS_NAME::INS_NAME(Register dest, Register reg, Register val)             \
        : BinaryArithmeticInstruction(OP, dest, reg, val) { }
...
void FloatBinaryArithmeticInstruction::execute(
    ReservationStation::Entry& entry) const {
    const auto& operands = entry.operands();
    assert(operands.size() == 2);
    Alu::FloatResult binOpRes
        = op_(operands[0].getFloatValue(), operands[1].getFloatValue());
    entry.setFloatRegister(fReg_, binOpRes.value);
    entry.setFlags(binOpRes.flags);
}

#define FLOAT_BINARY_ARITH_INS_IMPL(INS_NAME, OP)                             \
    INS_NAME::INS_NAME(FloatRegister fReg, double val)                        \
        : FloatBinaryArithmeticInstruction(OP, fReg, val) { }                 \
    INS_NAME::INS_NAME(FloatRegister fReg, FloatRegister val)                 \
        : FloatBinaryArithmeticInstruction(OP, fReg, val) { }                 \
    INS_NAME::INS_NAME(FloatRegister fReg, Operand val)                       \
        : FloatBinaryArithmeticInstruction(OP, fReg, val) { }
...
void UnaryArithmeticInstruction::execute(
    ReservationStation::Entry& entry) const {
    const auto& operands = entry.operands();
...
#define UNARY_ARITH_INS_IMPL(INS_NAME, OP)                                    \
    INS_NAME::INS_NAME(Register reg)                                          \
        : UnaryArithmeticInstruction([](int64_t value) { return OP; }, reg) { \
    }
...
std::vector<Operand> MOV::operands() const {
    if (destination_.isRegister() || destination_.isMemoryImmediate()
        || destination_.isFloatRegister()) {
        return { value_ };
        ...
        return { value_,
            destination_.getMemoryRegisterOffset().regOffset().reg() };
    } else if (destination_.isMemoryRegisterScaled()) {
        return { value_,
            destination_.getMemoryRegisterScaled().regScaled().reg() };
        ...
        const auto& regOffsetReg
            = destination_.getMemoryRegisterOffsetRegister().regOffsetReg();
        return { value_, regOffsetReg.regOffset().reg(), regOffsetReg.reg() };
    } else if (destination_.isMemoryRegisterRegisterScaled()) {
        const auto& regRegScaled
            = destination_.getMemoryRegisterRegisterScaled().regRegScaled();
        return { value_, regRegScaled.regScaled().reg(), regRegScaled.reg() };
    } else if (destination_.isMemoryRegisterOffsetRegisterScaled()) {
        const auto& regOffsetRegScaled
            = destination_.getMemoryRegisterOffsetRegisterScaled()
                  .regOffsetRegScaled();
        return { value_, regOffsetRegScaled.regScaled().reg(),
            regOffsetRegScaled.regOffset().reg() };
    }
...
void MOV::execute(ReservationStation::Entry& entry) const {
    ...
    if (destination_.isRegister()) {
        ...
        assert(operands.size() == 1);
        entry.setFloatRegister(
            destination_.getFloatRegister(), operands[0].getFloatValue());
        ...
        if (destination_.isMemoryRegister()) {
            ...
            address = Operand::supply(
                destination_.getMemoryRegister(), operands[1].getValue())
                          .index();
        } else if (destination_.isMemoryRegisterOffset()) {
            assert(operands.size() == 2);
            address = Operand::supply(
                destination_.getMemoryRegisterOffset(), operands[1].getValue())
                          .index();
        } else if (destination_.isMemoryRegisterScaled()) {
            assert(operands.size() == 2);
            address = Operand::supply(
                destination_.getMemoryRegisterScaled(), operands[1].getValue())
                          .index();
        } else if (destination_.isMemoryRegisterRegister()) {
            assert(operands.size() == 3);
            address = Operand::supply(
                Operand::supply(destination_.getMemoryRegisterRegister(),
                    operands[1].getValue()),
                operands[2].getValue())
                          .index();
        } else if (destination_.isMemoryRegisterOffsetRegister()) {
            assert(operands.size() == 3);
            address = Operand::supply(
                Operand::supply(destination_.getMemoryRegisterOffsetRegister(),
                    operands[1].getValue()),
                operands[2].getValue())
                          .index();
        } else if (destination_.isMemoryRegisterRegisterScaled()) {
            assert(operands.size() == 3);
            address = Operand::supply(
                Operand::supply(destination_.getMemoryRegisterRegisterScaled(),
                    operands[1].getValue()),
                operands[2].getValue())
                          .index();
        } else if (destination_.isMemoryRegisterOffsetRegisterScaled()) {
...
void ConditionalJumpInstruction::retire(
    ReservationStation::Entry& entry) const {
    const auto& operands = entry.operands();
    assert(operands.size() == 2);
...
void ConditionalJumpInstruction::execute(
    ReservationStation::Entry& entry) const {
    const auto& operands = entry.operands();
    assert(operands.size() == 2);
...
#define COND_JMP_INS_IMPL(INS_NAME, CONDITION)                                \
    INS_NAME::INS_NAME(Register address)                                      \
        : ConditionalJumpInstruction(                                         \
            [](Alu::Flags flags) { return CONDITION; }, address) { }          \
    INS_NAME::INS_NAME(uint64_t address)                                      \
        : ConditionalJumpInstruction(                                         \
            [](Alu::Flags flags) { return CONDITION; }, address) { }          \
    INS_NAME::INS_NAME(Memory::Immediate address)                             \
        : ConditionalJumpInstruction(                                         \
            [](Alu::Flags flags) { return CONDITION; }, address) { }          \
    INS_NAME::INS_NAME(Memory::Register address)                              \
        : ConditionalJumpInstruction(                                         \
            [](Alu::Flags flags) { return CONDITION; }, address) { }          \
    INS_NAME::INS_NAME(Memory::RegisterOffset address)                        \
        : ConditionalJumpInstruction(                                         \
            [](Alu::Flags flags) { return CONDITION; }, address) { }          \
    INS_NAME::INS_NAME(Operand address)                                       \
        : ConditionalJumpInstruction(                                         \
            [](Alu::Flags flags) { return CONDITION; }, address) { }
...
COND_JMP_INS_IMPL(
    JG, !flags.zeroFlag && (flags.signFlag == flags.overflowFlag))
...
void CMP::execute(ReservationStation::Entry& entry) const {
    ...
    assert(operands.size() == 2);
    Alu::Result res
        = Alu::subtract(operands[0].getValue(), operands[1].getValue());
    entry.setFlags(res.flags);
}
...
void FCMP::execute(ReservationStation::Entry& entry) const {
    const auto& operands = entry.operands();
    assert(operands.size() == 2);
    Alu::FloatResult res = Alu::fsubtract(
        operands[0].getFloatValue(), operands[1].getFloatValue());
    entry.setFlags(res.flags);
}
...
void PatchableJumpInstruction::setDestination(uint64_t address) {
    assert(address_.isValue()
        && static_cast<uint64_t>(address_.getValue()) == Label::empty()
        && "You can change jump destination only on empty labeled jumps");
    address_ = address;
}

void LEA::validate() const {
    if (reg_.isSpecial() || reg_ == Register::StackPointer()
        || reg_ == Register::StackBasePointer()) {
        throw InvalidOperand(reg_);
    }
}
...
void LEA::execute(ReservationStation::Entry& entry) const {
    ...
    if (mem_.isMemoryRegisterOffset()) {
        assert(operands.size() == 1);
        address = Operand::supply(
            mem_.getMemoryRegisterOffset(), operands[0].getValue())
                      .index();
    } else if (mem_.isMemoryRegisterScaled()) {
        assert(operands.size() == 1);
        address = Operand::supply(
            mem_.getMemoryRegisterScaled(), operands[0].getValue())
                      .index();
    } else if (mem_.isMemoryRegisterRegister()) {
        assert(operands.size() == 2);
        address
            = Operand::supply(Operand::supply(mem_.getMemoryRegisterRegister(),
                                  operands[0].getValue()),
                operands[2].getValue())
                  .index();
    } else if (mem_.isMemoryRegisterOffsetRegister()) {
        assert(operands.size() == 2);
        address = Operand::supply(
            Operand::supply(mem_.getMemoryRegisterOffsetRegister(),
                operands[0].getValue()),
            operands[1].getValue())
                      .index();
    } else if (mem_.isMemoryRegisterRegisterScaled()) {
        assert(operands.size() == 2);
        address = Operand::supply(
            Operand::supply(mem_.getMemoryRegisterRegisterScaled(),
                operands[0].getValue()),
            operands[1].getValue())
                      .index();
    } else if (mem_.isMemoryRegisterOffsetRegisterScaled()) {
        assert(operands.size() == 2);
        address = Operand::supply(
            Operand::supply(mem_.getMemoryRegisterOffsetRegisterScaled(),
                operands[0].getValue()),
            operands[1].getValue())
                      .index();
    } else {
