...
public:
    ...
    /*
      void InitSizes() {
          ...
          data_size =
      std::stoull(process->Receive()->substr(strlen("DATASIZE:")));
  ...
  */
    ...
    /// respected.
    void WriteText(
        uint64_t address, const std::vector<std::string>& data) override {
    ...
    std::vector<std::string> ReadText(
        uint64_t address, size_t amount) override {
    ...
    std::vector<uint64_t> ReadMemory(
        uint64_t address, size_t amount) override {
    ...
    void SetRegisters(const std::map<std::string, int64_t>& regs) override {
        for (const auto& [name, val] : regs) {
            if (!IsValidRegisterName(name)) {
                throw DebuggerError(
                    fmt::format("Register name '{}' is not valid!", name));
            }
    ...
    void Wait() override {
        ...
        if (!message || message != "STOPPED") {
            throw DebuggerError(
                fmt::format("Expected STOPPED message in Wait()"));
        }
    }
...
private:
    ...
    bool IsValidRegisterName(std::string_view name) {
        return name == "IP" || name == "BP" || name == "SP" || name == "FLAGS"
            || isGPRegister(name);
    }
