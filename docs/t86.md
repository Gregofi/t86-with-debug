...
## T86-CLI
This is the preffered and more supported method.

The CLI accepts a T86 executable in a text format and runs it. The executable must have the following format:
The executable is made up of sections. The T86-CLI cares about the `.text` and `.data` sections.
The executable also follows line comments, beginning with `#`.

### Text section
Contains the instructions that will be run by the T86 VM.
...
Do note that most instructions have very limited set of
...
### Data section
Contains a data that will be stored in the RAM beginning from address 0 upwards
...
### Examples
See the `t86/t86-cli/tests` for some more advanced T86 executables.
For even more advanced features and included debug sections,
see the `t86/dbg-cli/tests`. Documentation about debug sections
can be found in `docs/debugging.md`, they are however not needed
for just running the program.

### Running the program
You need to built the project (see `README.md`). After that, just run `t86-cli/t86-cli <executable>`.
...
note that it is not as supported as the CLI.
